...
class ProStatsFetcher:
    ...
    def _find_all_games_on_date(
        ...
        collect every game instead of picking one.  Same MLB→MiLB→Breakout→parent
        fallback chain.
        ...
        try:
            ...
            if api_team_id:
                ...
                if team_info["sport_id"] == 1:
                    ...
                    game = self._match_team_in_schedule(
                        milb_schedule, cached_name, player_id,
                    )
                    if game:
                        return game
            ...
            if player_id:
                ...
                if api_team_id:
                    ...
                    if team_info["sport_id"] == 1:
                        ...
                        games = self._match_all_in_schedule(
                            milb_schedule, cached_name, None,
                        )
                        if games:
                            return games
    ...
    def _match_team_in_schedule(
        ...
        for game in schedule:
            ...
            if home_match or away_match:
                ...
                # Tag exhibition / Spring Breakout games
                is_exhibition = game.get("game_type") == "E"
                matches.append({
        ...
        # Multiple matches: prefer the game where the player appears in the boxscore
    ...
    def _match_all_in_schedule(
        ...
        # Multiple matches: return every game where the player appears in the boxscore
        found = []
        for match in matches:
            box = match["boxscore"]
            if not box:
                ...
                for search_name, sport_id in search_targets:
                    ...
                    for game in schedule:
                        ...
                        if search_name in home or search_name in away:
                            ...
                            if cache_key:
                                self._next_game_cache[cache_key] = result
                            return result
            if cache_key:
                self._next_game_cache[cache_key] = None
            return None
        except Exception:
